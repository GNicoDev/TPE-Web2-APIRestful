...
class VehicleController
{
    ...
    function getAll($req, $res)
    {
        ...
        if (isset($req->query->orderDir)) {
            $orderDir = $req->query->orderDir;
            if ($orderDir != 'ASC' && $orderDir != 'DESC') {
                return $this->view->response("Invalid order direction. Use 'ASC' or 'DESC'.", 400);
            }
        } else
            $orderDir = 'ASC';

        $vehicles = $this->vehicleModel->getAllVehicles($orderBy, $orderDir);
        return $this->view->response($vehicles);
    }
    ...
    function create($req, $res)
    {
        ...
        if (
            empty($body->marca) || empty($body->modelo) || empty($body->matricula)
